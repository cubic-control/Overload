...
public class ChunkProviderRainbow implements IChunkProvider
{
                    ...
                    for (int l2 = 0; l2 < 8; ++l2)
                    {
                        ...
                        for (int i3 = 0; i3 < 4; ++i3)
                        {
                            ...
                            for (int k3 = 0; k3 < 4; ++k3)
                            {
                                if ((d15 += d16) > 0.0D)
                                {
                                    p_147424_3_[j3 += short1] = MBlocks.black_block;
                                }
                                else if (k2 * 8 + l2 < b0)
                                {
                                    p_147424_3_[j3 += short1] = MBlocks.blue_block;
                                }
                                else
                                {
    ...
    public void populate(IChunkProvider par1IChunkProvider, int par2, int par3)
    {
            ...
            && TerrainGen.populate(par1IChunkProvider, worldObj, rand, par2, par3, flag, LAKE))
        {
            ...
            (new WorldGenLakes(MBlocks.blue_block)).generate(this.worldObj, this.rand, k1, l1, i2);
        }
        ...
        if (TerrainGen.populate(par1IChunkProvider, worldObj, rand, par2, par3, flag, LAVA) && !flag && this.rand.nextInt(8) == 0)
        {
            ...
            {
                (new WorldGenLakes(MBlocks.cyan_block)).generate(this.worldObj, this.rand, k1, l1, i2);
            }
        }
        ...
        doGen = TerrainGen.populate(par1IChunkProvider, worldObj, rand, par2, par3, flag, FIRE);
        for (k1 = 0; doGen && k1 < 16; ++k1)
        {
            for (l1 = 0; l1 < 16; ++l1)
            {
                ...
                {
                    this.worldObj.setBlock(k1 + k, i2 - 1, l1 + l, MBlocks.cyan_block, 0, 2);
                }

                if (this.worldObj.func_147478_e(k1 + k, i2, l1 + l, true))
                {
                    this.worldObj.setBlock(k1 + k, i2, l1 + l, MBlocks.white_block, 0, 2);
                }
            }
        }
